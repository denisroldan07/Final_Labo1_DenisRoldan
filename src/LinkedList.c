...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this))
    {
    	...
    	if(pNodeNew != NULL)
    	{
    		...
    		{
    			pNodeAfter = getNode(this,nodeIndex - 1);

    			if(pNodeAfter != NULL)
    			{
    				pNodeNew->pNextNode = pNodeAfter->pNextNode;
    				pNodeAfter->pNextNode = pNodeNew;
    			}
    		}
    		this->size++;
    		returnAux = 0;
...
{
    void* returnAux = NULL;
    void* punteroAux = NULL;

    if(this != NULL && index >= 0 && index <= ll_len(this))
    {
    	punteroAux = ll_get(this, index);

    	if(!ll_remove(this, index))
		{
			returnAux = punteroAux;
		}
    }
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    if(this != NULL && from >= 0 && to > 0 && to > from && from <= ll_len(this) && to <= ll_len(this))
    {
...
LinkedList* ll_mapL(LinkedList* this , void(*pFunc)(void* pElement))
{
	...
	printf("SE AGREGARON CORRECTAMENTE LOS PROMEDIOS !\n");
	return this;
}
